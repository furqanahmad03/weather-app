...
$('#country').on('input', () => {
  ...
  if (filteredStates.length > 0) {
    filteredStates.slice(0, 7).forEach((state) => {
      const litElement = $('<li>');
...
let populateData = function (url, city_name) {
  let response = fetch(url)
    ...
    .then((response) => {
      ...
      $(".visibility").html(response["visibility"] / 1000 + " km");
      ...
      let date = new Date();
       if (date.getHours() >= 6 && date.getHours() <= 18) {
        ...
        else if (response["weather"]["0"]["main"].includes("Rain") || response["weather"]["0"]["main"].includes("Drizzle")) {
          $('#rain').css('display', 'block');
        }
        else if (response["weather"]["0"]["main"].includes("Clouds") || (parseFloat(response["main"]["temp"]) - 273.15).toFixed(1) < 10) {
          $('#sun-cloud').css('display', 'block');
        }
        else {
      ...
      if (response["main"]["pressure"] <= 1013) {
        $('.pressure-high').css('display', 'none');
        $('.pressure-low').css('display', 'block');
      }
      else {
        $('.pressure-low').css('display', 'none');
        $('.pressure-high').css('display', 'block');
      }
...
$('.search-history').on('click', '.deleteItem', function (e) {
  e.preventDefault();
